...
**Rebuild rule (single source of truth):** runner-images (`--profile runner-images`) rebuild **only** when something under [docker/](docker/) changed (any of the six `docker/runner-*` image dirs); the auto-deploy greps `git diff` for `^docker/` and sets `REBUILD_RUNNER_IMAGES=1`, then [bin/pull-latest.sh](bin/pull-latest.sh) builds the **whole** `runner-images` profile (Docker layer cache makes unchanged images a no-op, so adding a language needs no script change). App services (`api` / `runner` / `frontend`) always rebuild on deploy.
