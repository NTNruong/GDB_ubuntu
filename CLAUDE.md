...
API: `RUNNER_BASE_URL` (HTTP) and `RUNNER_WS_URL` (WebSocket) must point at the same runner instance. Accounts/explorer (Phase 2): `USER_HOMES_ROOT` (per-user homes dir, host bind via `USER_HOMES_HOST_ROOT` in compose), `USERS_FILE` (defaults to `<USER_HOMES_ROOT>/users.json`), `SESSION_SECRET` (cookie signing — unset = ephemeral, sessions reset on restart), `SESSION_COOKIE_SECURE` (`1` to mark the cookie `Secure`). AI assistant (Phase 3): `AI_ENABLED` (`0` disables the local-llama backend), `LLAMA_BASE_URL` (host llama.cpp server, default `http://localhost:8000`), `GEMINI_API_KEY` (unset ⇒ Google backends hidden from the model list), `AI_DATA_ROOT` (per-user chat threads, separate from `USER_HOMES_ROOT`). Defaults in [apps/api/src/config.ts](apps/api/src/config.ts).
