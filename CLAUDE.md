...
Tailnet-only online code runner for **seven languages** — C (`gnu17`), C++ (`gnu++20`), Python 3.12, JavaScript (Node), Java (selectable JDK 17/21/25), Go, and Rust. Every language can **run**; **debugging** (DAP-based) is available for C/C++/Python/Go/Rust/Java — only JavaScript is run-only. (Java debug uses Eclipse JDT LS + Microsoft java-debug behind an in-container bridge; all other DAP languages use a lightweight gdb/debugpy/Delve adapter.) Multi-file projects are supported (per-language file extensions, a `files[]` array). Anonymous run/debug stays login-free and stateless — source/stdin/argv are redacted from logs and cleaned up per job. **Phase 2** adds optional **app-managed accounts** (bcrypt `users.json`, JWT-in-cookie sessions) that unlock a VSCode-like **file explorer** over a per-user home directory (`USER_HOMES_ROOT/<username>`, full CRUD + Ctrl+S save + run-the-folder). Accounts support **public self-registration → admin-approval** (new sign-ups land in `pending` until an admin approves), **roles** (`admin`/`user`), an account **lifecycle** (`pending`/`active`/`disabled`), optional **TOTP 2FA** (mandatory for admins), and **session revocation** without a server-side store (a `tokenVersion` baked into the cookie; bumping it on password-change / admin-reset / "log out everywhere" invalidates every outstanding cookie). The `users` CLI is still the headless path for ops + bootstrapping the first admin. Designed to be exposed only inside a tailnet; do not put behind the public internet without adding rate limiting (and a stable `SESSION_SECRET` + HTTPS for accounts).
...
- [apps/api/src/app.ts](apps/api/src/app.ts) is mostly a thin proxy for run/debug: validates with the shared zod schemas, forwards REST to the runner, and bidirectionally bridges client WebSockets to runner WebSockets (`/api/run/:id`, `/api/debug/:id`). It also re-streams the runner's SSE event stream at `/api/run/:id/events`. **Phase 2** adds locally-served, stateful plugins it registers (in `app.ts` order): [auth.ts](apps/api/src/auth.ts) (`/api/auth/*` register/login/logout/me/change-password; cookie-JWT + login lockout; decorates `app.authenticate` / `app.requireAdmin`), [admin.ts](apps/api/src/admin.ts) (`/api/admin/users/*`, all `requireAdmin`: approve/reject/role/status/reset-password/delete — guards against self-demote/self-disable/self-delete), [account.ts](apps/api/src/account.ts) (`/api/account/*` self-service: profile, `logout-all`, TOTP 2FA setup/enable/disable — acts only on `request.user.sub`), and [files.ts](apps/api/src/files.ts) (`/api/files/*`, auth-gated per-user CRUD; path safety in [pathSafety.ts](apps/api/src/pathSafety.ts)). Accounts live in [userStore.ts](apps/api/src/userStore.ts) (bcrypt + the `tokenVersion`/status/role machinery; legacy CLI-seeded records migrate to `active`/`user` on read); 2FA codes come from the dependency-free [totp.ts](apps/api/src/totp.ts) (RFC 6238). These plugins + [chat.ts](apps/api/src/chat.ts) are the stateful endpoints — they read/write `USER_HOMES_ROOT` / `AI_DATA_ROOT`, never the runner.
...
- **Persistence:** threads are per-user JSON under `AI_DATA_ROOT/<username>/` — a **separate root from `USER_HOMES_ROOT`** so chat history never appears in the file explorer ([walkTree](apps/api/src/pathSafety.ts) walks the whole home incl. dotfiles) and cannot be CRUD'd via `/api/files/*`. Reuses `resolveUserPath`/`assertSafePath` for thread-id safety. Each thread is a **branching tree** (llama.cpp-style): `AiThread.nodes[]` + `currentLeafId`; edit/regenerate create sibling variants and the rendered transcript is the path to the current leaf ([threads.ts](apps/api/src/ai/threads.ts) `pathToLeaf`/`addNode`/`deleteSubtree`/`setCurrentLeaf`; [aiTree.ts](apps/frontend/src/aiTree.ts) mirrors path/variant helpers). Legacy linear `messages[]` threads migrate to nodes on read (`migrateThread`).
