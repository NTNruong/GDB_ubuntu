...
export function createApiServer(config: ApiConfig): FastifyInstance {
  ...
  app.post<{ Body: unknown; Reply: JobCreateResponse | ErrorResponse }>("/api/run", async (request, reply) => {
  ...
  app.post<{ Body: unknown; Reply: JobCreateResponse | ErrorResponse }>("/api/debug", async (request, reply) => {
  ...
  app.get<{ Params: { id: string } }>("/api/run/:id", { websocket: true }, (client, request) => {
